...
# import run program
from run_scripts.run import run as run
from run_scripts.collect_data import run as collect_data
from run_scripts.offline_run import run as offline_run
from run_scripts.multi_task_offline_run import run as mto_run
...
def _get_run_file(params):
    ...
    for _i, _v in enumerate(params):
        ...
        #     return run_file
        match _v:
            case "--run" | "--collect" | "--offline" | "--mto" | "--ro":
                run_file = _v[2:] # remove "--"
                params.remove(_v)
                break
    
    if run_file == '': 
        run_file = 'run'
        logger.warning("No run file specified, use 'run' (online) as the default.")            
    return run_file
...
if __name__ == '__main__':
    ...
    if 'seed' not in config_dict: config_dict['seed'] = 123
