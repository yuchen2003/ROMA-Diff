...
def run(_run, _config, _log):
    ...
    match args.offline_data_quality.lower():
        ...
        case _:
            raise ValueError(
                "Dataset quality {} not supported!".format(args.offline_data_quality)
            )
...
def evaluate_sequential(args, runner, logger):
    ...
    logger.console_logger.info(
        "Do offline data collection with collecting {} trajectories with checkpoint loaded from {}".format(
            args.num_episodes_collected, args.checkpoint_path
        )
    )
...
def run_sequential(args, logger):
    ...
    if args.save_replay_buffer and args.offline_data_quality.lower() != "random":
        ...
        match args.env:
            case "sc2":
                save_path = os.path.join(
                    args.offline_data_folder,
                    args.env,
                    args.env_args["map_name"],
                    args.offline_data_quality + "-replay",
                    args.unique_token,
                )
            case "gymma":
            ...
            case _:
                raise NotImplementedError(
                    "Do not support such envs: {}".format(args.env)
                )
    ...
    logger.console_logger.info(
        "collect data with data quality: {}".format(args.offline_data_quality)
    )
    ...
    while runner.t_env < args.t_max:
        ...
        with th.no_grad():
            ...
            if (
                ...
                and args.offline_data_quality.lower() != "random"
            ):
                replay_saver.append(
                    data={
                        k: episode_batch[k].clone().cpu()
