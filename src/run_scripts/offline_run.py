...
def run(_run, _config, _log):
    ...
    logger = Logger(_log)
    _log.info("Offline data quality: ", args.offline_data_quality)
...
def run_sequential(args, logger):
    ...
    offline_buffer = OfflineBuffer(
        ...
        args.offline_bottom_data_path,
        args.offline_max_buffer_size,
        shuffle=args.offline_data_shuffle,
    )  # device default cpu

    logger.console_logger.info(
        "Beginning  offline training with {} iterations".format(args.t_max)
    )
    ...
    if args.save_model:
        ...
        logger.console_logger.info(
            "Save final model checkpoint in {}".format(save_path)
        )
...
def train_sequential(args, logger, learner, runner, offline_buffer):
    ...
    with th.no_grad(): # [?] what's the effect
        runner.t_env = t_env
    ...
    while t_env < t_max: 
        ...
        if (t_env - last_test_T) / args.test_interval >= 1 or t_env >= t_max:
            ...
            with th.no_grad():
                runner.t_env = t_env
                for _ in range(n_test_runs):
                    runner.run(test_mode=True) # Evaluate
            test_time_total += time.time() - test_start_time

            logger.console_logger.info("Step: {}/{}".format(t_env, t_max))
            logger.console_logger.info(
                "Estimated time left: {}. Time passed: {}. FPS {:.2f}. Test time cost: {}".format(
                    time_left(last_time, last_test_T, t_env, t_max),
                    time_str(time.time() - start_time),
                    (t_env - last_test_T) / (time.time() - last_time),
        ...
        if args.save_model and (
            t_env - model_save_time >= args.save_model_interval or model_save_time == 0
        ):
